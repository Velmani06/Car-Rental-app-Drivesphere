...
function SortingMenu({ onSortChange }) {
  ...
  const handleSortItemClick = (itemName) => {
    ...
    setSortMenuVisible(false);
    onSortChange(itemName); // ✅ Send selected option upward
  };
  ...
  return (
    <div className="sorting w-[15rem] h-12 bg-white relative">
      <div className="flex justify-start items-center h-full">
        ...
        <div
          className="sorting-menu-container w-[10rem] h-10 bg-[#F5F5F5] flex justify-center items-center gap-2 cursor-pointer"
          onClick={toggleSortMenu}
          ref={menuRef}
        >
          <p className="text-[#7a7777]">{sortName}</p>
          ...
          <ul className="bg-white">
            {sortingList.map((list, index) => (
              <li
                ...
                className="w-full h-10 flex items-center mb-2 pl-8 hover:bg-[#DC2D13] hover:text-white cursor-pointer"
                key={index}
              >
                {list.listName}
              </li>
